...
If ::oProdAutor:ExisteProd(cCodBarra)
	...
	If ::oProdAutor:PodeVender(cCodBarra, nQtde)
		...
		If nPrcLiqLj > oProdAut:nVlUnVenda
			...
			lItemPbm	:= .T.
			lRet		:= .T.
			LjGrvLog( NIL, " Obtido valor de desconto do produto [C๓digo de barras: " + cLogProd + "]  - Valor Desconto: ",nPercDesc )
		Else
			lRet := .T.
		Endif
    Else
    	lRet := .F.
    	LjGrvLog( NIL, " Nใo pode vender o produto [C๓digo de barras: " + cLogProd + "]  " )
    EndIf
Else
	LjGrvLog( NIL, " Nใo encontrou o produto [C๓digo de barras: " + cLogProd + "]  para ser autorizado " )
	lRet := .T.
EndIf
...
If lRetorno
	...
	Else
		...
		MsgAlert(STR0008,"EPHARMA") //"Lista de produtos EPHARMA nใo retornada no servicoX"
		lRetorno := .F.
	EndIf
Else
