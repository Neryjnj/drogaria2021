		...
		If ValType(oPBM:oPBM:oPBM:oSitefPBM:oClisitef:oRetorno) == "O"
			...
			For I := 1 to oPBM:oPBM:oPBM:oSitefPBM:oClisitef:oRetorno:nQtdeMed
				...
				Else
					...
					//e diferenÁa de preÁo na tela de PBM, portanto comento o cÛdigo abaixo
					//e deixo para que se futuramente esse valor queira ser utilizado j· est· aqui
...
If lTotvsPDV
	...
	lRet := IIF(ValType(lRet) <> "L", .T., lRet) //Caso o fonte esteja desatualizado, trato pois a funÁ„o n„o retornava nenhum valor
	If lRet
...
If lTotvsPDV
	//N„o ser· desenvolvido nessa etapa do projeto pois o cliente 
	//n„o usa esse PBM, posteriormente dever·, se necess·rio, ser desenvolvido 
	LjGrvLog(NIL,"OperaÁ„o PHARMASYSTEM_CANCELAMENTO n„o integrada para TOTVSPDV")
Else
...
Conout("DROVLBPro - Busca do Produto - CÛdigo de Barras -> " + cCodBarra)
...
---------------------------------------------------------------------------------------------
±±≥DescriáÑo ≥ Rotina chamada a partir do VidaLink atravez de integraÁ„o via DLL.         ≥±±
...
Template Function DROVLCall(cFuncao, uParm1, uParm2, uParm3, uParm4, uParm5, uParm6)
	...
	Conout("DroVlCall - Inicio da FunÁ„o")
	...
	If lConnect
		If lNewConnect
			...
			Conout("DROVLCall - Prepara nova conex„o")
		EndIf
		...
		If Empty(AllTrim(cRet))
			ConOut("Retorno: **Produto n„o encontrado** ")
		Else
	...
	Conout("DroVlCall - Final da FunÁ„o <- Retorno [" + cRet + "]")
Return cRet
...
If Len(aProd) > 0
	For nX := 1 To Len(aProd)
		...
		Else		 
			LjGrvLog( "PBM", "Produto n„o inserido aProd[nX][VL_EAN] " , aProd[nX][VL_EAN])
		EndIf
	Next nX	
	...
	If lTotvsPDV
		...
		//e permite lanÁamento de outros itens que n„o s„o da PBM
		If lRet
			STFRefTot()
		...
		Else
			LjGrvLog( "PBM", "Erro no lanÁamento dos itens da PBM na Grade de vendas")
		EndIf
	Else
...
/*/{Protheus.doc} DroRtOtran
	...
	@param lCancTotal, lÛgico, efetua cancelamento: total (.T.), parcial (.F.)
...
Else
	//FaÁo o tratamento do cÛdigo 561 - FuncionalCard pois internamente para os
	//tratamentos do SITEF deve considerar 560 sen„o n„o finaliza a PBM,
