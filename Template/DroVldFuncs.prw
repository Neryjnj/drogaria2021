...
If lFabric // verifica se os campo Fabricante e Loja estao preenchidos 
	...
	If SA2->(!MsSeek(xFilial("SA2")+cCodFab+cCodLoja))// verifica se o codigo do fornecedor + codigo da loja existem em SA2
		...
		lRet := .F.
	Else
		If SA2->A2_FABRICA = "N" .Or. SA2->A2_FABRICA = "2"//caso codigo do fabricante + codigo da loja existam,verifica se a opcao A2_FABRICA = "N" ou "2" (No Template de Drogaria o Conteudo do campo A2_FABRICA era S=Sim; N=Nao, no padrใo assumimos 1=Fabricante; 2=Fornecedor)
			MsgAlert(cText2)
			lRet := .F.
		Endif
	Endif
Else
	MsgAlert("Este campo deve ser preenchido com um fabricante/fornecedor vแlido." + CHR(13) + CHR(13) +;
			"Verifique se o campo Fabricante(A2_FABRICA) do cadastro de Fornecedor estแ como 'Fabricante' ")
Endif
...
If lDadosTela .OR. lF12
	...
	If nLinha == 0 //senใo abaixo darแ errorlog
...
If T_DroLenANVISA() > 0 
	For nCont := 1 to T_DroLenANVISA()
		If !(ValType(aANVISA[nCont][PRODUTO]) $ "C|N")
			LjGrvLog(aANVISA[nCont][NUMDOC],"TPL Drogaria Item do Array Anvisa sem produto preenchido serแ desconsiderado")
			Conout("TPL Drogaria - DOC [" + cValToChar(aANVISA[nCont][NUMDOC]) + "] - Item do Array Anvisa sem produto preenchido serแ desconsiderado")
			Loop
		EndIf
		...
		If lTotvsPDV
			REPLACE LK9_USVEND  WITH IIF(!Empty(STDGPBasket("SL1","L1_USVENDA")),STDGPBasket("SL1","L1_USVENDA"),cUserName)	//No F4, nใo preciso mais digitar usuแrio e a senha do supervisor, somente o login do usuแrio.
...
Local lTotvsPDV := STFIsPOS()
Local lMvLjPdvPa:= IIf(lTotvsPDV,STBIsPAF(),LjxBGetPaf()[2]) //Indica se ้ pdv
Local xRet		:= NIL  
...
If HasTemplate("DRO") .Or. LjIsDro()
	lRet := .T.
Else
	Final("Acesso negado pois o CNPJ nใo esta liberado para uso de Template de Drogaria")
EndIf
...
If !lProfile42
	lRet := .F.
	cMensagem := "Usuแrio nใo ้ um Farmaceutico. Venda/Cadastro nใo permitido"
	MsgAlert(cMensagem)
EndIf
...
Local lProfile42:= .F.
Local lTotvsPDV := STFIsPOS()
Local cMsg 		:= "Usuแrio nใo ้ um Farmaceutico. Nใo poderแ efetuar manuten็ใo neste cadastro."
Local aProfile42:= {}
...
Local lRet 		:= .F.
Local nPos		:= 0
Local cCNPJ		:= ParamIxb[1] //N๚mero do CNPJ (M0_CGC)
